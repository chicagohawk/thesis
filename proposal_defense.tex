...
\noindent Fourthly, we need to extend the basis selection scheme to time-independent PDEs.
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item $g\in\mathcal{H}^\prime$ be the noisy gradient of $f$. 
        \item $\epsilon$ be the gradient sample noise
              $$
                  \epsilon = g - \nabla f\,.
              $$
    \end{enumerate}
...
\label{examples}
Gas turbine blades are designed to operate at high temperatures.
...
%    $\|\lambda_1-\lambda_2\|$ is bounded.
    \section{Proof of theorem \ref{theorem: 2}: optimization convergence}
